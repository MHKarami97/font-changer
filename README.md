...
## به‌روزرسانی نسخه ۱.۳.۰ - ریشه‌یابی دقیق‌تر مشکل آیکون‌ها

### چرا حتی بعد از رفع Shadow DOM، مشکل ادامه داشت؟

بعد از بررسی دقیق‌تر، دو علت جداگانه شناسایی شد که هر دو باید هم‌زمان رفع
می‌شدند:

**علت ۱: ترتیب اجرا (Race Condition)**
در نسخه قبلی، فونت سراسری *قبل* از اسکن آیکون‌ها اعمال می‌شد. وقتی
`IconFontDetector` بعداً `getComputedStyle` هر المان را می‌خواند، فونتی که
می‌دید همان فونت جدید (مثلاً Vazirmatn) بود، نه فونت اصلی آیکون؛ در نتیجه
هیچ‌کدام از کلیدواژه‌های آیکون (`google symbols`, `material icons`, ...)
تطبیق پیدا نمی‌کرد. **راه‌حل:** ترتیب اجرا برعکس شد؛ الان ابتدا آیکون‌ها
اسکن و علامت‌گذاری می‌شوند، سپس فونت سراسری اعمال می‌شود.

**علت ۲: رندر آیکون از طریق Pseudo-element**
بسیاری از سیستم‌های آیکون گوگل (از جمله Google Symbols در Google Maps)
گلیف آیکون را با ویژگی CSS به نام `content` روی `::before` یا `::after`
رندر می‌کنند، نه مستقیم روی خود المان. جاوااسکریپت به‌هیچ‌وجه نمی‌تواند
`element.style` را برای یک Pseudo-element تنظیم کند، چون `::before`/`::after`
اصلاً Node مستقلی در DOM نیستند. بنابراین قفل inline نسخه قبلی، که فقط روی
خود المان اثر داشت، تاثیری روی گلیف داخل `::before` نداشت.

**راه‌حل:** `IconFontDetector` حالا علاوه بر خود المان، `getComputedStyle(element, '::before')`
و `getComputedStyle(element, '::after')` را هم بررسی می‌کند. اگر آن
Pseudo-element محتوای واقعی (`content`) دارد و فونتش آیکونی تشخیص داده شود،
یک شناسه یکتا (`data-fc-icon-id`) به المان اصلی اختصاص داده می‌شود و یک
قانون CSS واقعی و مجزا مثل زیر تولید و در یک `<style>` جداگانه تزریق
می‌شود:

```css
[data-fc-icon-id="7"]::before {
  font-family: "Google Symbols" !important;
}
```

چون این selector (Attribute Selector) از specificity بالاتری نسبت به
selector سراسری `*` برخوردار است، و علاوه بر آن *بعد* از قانون فونت
سراسری در DOM قرار می‌گیرد، طبق قوانین Cascade در CSS همیشه برنده می‌شود
و فونت اصلی آیکون دست‌نخورده باقی می‌ماند.

### خلاصه تغییرات فنی

| بخش | قبل | بعد |
|---|---|---|
| ترتیب اجرا | فونت → اسکن آیکون | اسکن آیکون → فونت |
| محل بررسی فونت | فقط خود المان | خود المان + `::before` + `::after` |
| روش محافظت | `element.style.setProperty(...)` (روی Pseudo اثر ندارد) | قانون CSS با selector اختصاصی برای هر Pseudo |
